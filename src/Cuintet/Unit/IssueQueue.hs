module Cuintet.Unit.IssueQueue (IssueQueueReq (..), IssueQueueResp (..), issueQueue) where

import Clash.Prelude
import Cuintet.CoreCtrl (InstCtrl, NExecUnits, OpClass, execUnit, fitsPort, opClassOf, usesRs1, usesRs2)
import Cuintet.Eei (Addr, DispatchWidth, IssueWidth, LoadQueueAddr, NAluPorts, NPRegs, NRob, PRegAddr, RobAddr, StoreQueueAddr, TrapCause, XLen)
import Cuintet.Pipeline (Renamed (..))
import Cuintet.Unit.Btb (Prediction)
import Cuintet.Unit.MultiRam (multiRam)
import Cuintet.Util (oldest, orNothing, (<<$>>))

type NBroadcast = NAluPorts + NExecUnits
...
without :: RobAddr -> Vec n (Maybe (RobAddr, Tag)) -> Vec n (Maybe (RobAddr, Tag))
...
step :: IssueQueueState -> IssueQueueReq -> (IssueQueueState, Vec IssueWidth (Maybe (RobAddr, Tag)))
step IssueQueueState {..} IssueQueueReq {..} = (IssueQueueState {tags = tags', ready = ready'}, selected)
  where
...
issueQueue req = mkOut <$> selection <*> multiRam (addrs <$> selection) (writes <$> req)
  where
    mkOut selected payloads = IssueQueueResp {issue = zipWith toRenamed payloads selected}
    toRenamed Payload {..} = fmap $ \(robAddr, Tag {..}) -> Renamed {..}

    selection = mealy step initState req

    addrs = fmap (maybe 0 fst)

    writes IssueQueueReq {dispatch} = payloadOf <<$>> dispatch
    payloadOf Renamed {..} = (robAddr, Payload {..})
